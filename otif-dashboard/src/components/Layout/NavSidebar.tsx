...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    expandable: true,
    children: [
      { id: 'collab-requests', label: 'My Requests', path: '/support/requests' },
    ],
  },
  {
    id: 'purchase',
...
function NavSidebar() {
  const [expandedItems, setExpandedItems] = useState<string[]>(['dashboards', 'collaboration'])
    ...
    <div className="w-56 bg-white border-r border-gray-200 h-screen fixed left-16 top-0 z-40 overflow-y-auto">
      <div className="py-4 px-3">
        <nav className="space-y-0.5">
          {menuItems.map((item) => {
            const isCollaboration = item.id === 'collaboration'
            const isCollabActive = location.pathname.startsWith('/support')

            return (
            <div key={item.id}>
              <button
                onClick={() => item.expandable ? toggleExpand(item.id) : item.id === 'collaboration' && navigate('/support')}
              ...
              >
                ...
                {isCollaboration && collabUnread > 0 && (
