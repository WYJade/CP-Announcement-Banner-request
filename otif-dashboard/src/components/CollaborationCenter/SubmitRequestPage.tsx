        ...
        <div className="p-6">
          ...
          {step === 'topic' && (
            <div className="space-y-4">
              ...
              {/* Invoice / Billing selection */}
              <div className="pt-4 border-t border-gray-100">
                <label className="block text-sm font-medium text-gray-700 mb-2">Invoice / Billing</label>
                <div className="flex gap-2 max-w-md">
                  {([
                    { key: 'none', label: 'No billing' },
                    { key: 'invoice', label: 'Invoice me' },
                    { key: 'prepaid', label: 'Prepaid balance' },
                  ] as const).map(({ key, label }) => (
                    <button
                      key={key}
                      type="button"
                      onClick={() => handle('billingOption', key)}
                      className={`flex-1 py-2.5 text-sm font-medium rounded-lg border transition-all ${
                        form.billingOption === key
